...
### Added
- **Hermes sessions in Recent Tasks.** Hermes/TARS stores history in SQLite (`$HERMES_HOME/state.db` or `~/.hermes/state.db`), not a prompt jsonl, so those sessions never appeared next to Claude/Grok. The collector now reads session titles (falling back to a 280-character redacted user excerpt), skips `cron`/archived/hidden rows, and resumes with `hermes --resume <id>`.
