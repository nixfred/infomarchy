...
describe("web mode rendering", () => {
  test("escapes HTML and strips identity fields", () => {
    ...
    // GITEA is a desktop card; its login never reaches a browser.
    expect(masked.ai.gitea).toBeUndefined();
  ...
  test("privacy masks recent-task prompts the same way as the desk", () => {
    ...
    const html = renderPage(snap, "/t/" + token + "/", "", parseDashPrefs({ privacyMode: true }), parseThemeColors(""), false);
...
describe("live listen", () => {
  test("refresh follows desktop privacy, prefs cannot unmask, revocation fails closed", async () => {
    ...
    try {
      ...
      // Privacy is off unless the desk saved a literal true, so browser and desk agree on every file state.
      for (const raw of ['{}', '{"privacyMode":"true"}', '{malformed']) {
        writeFileSync(join(dir, "dashboard.json"), raw);
        expect((await (await request()).text()).includes("PRIVATE_PROMPT_SENTINEL")).toBe(true);
      }
      writeFileSync(join(dir, "dashboard.json"), '{"privacyMode":true}');
